  ...
  group('HighchartsChart', () {
    ...
    testWidgets('Highcharts constructor', (WidgetTester tester) async {
      try {
        ...
        expect(chart.view.webView, isNotNull);
      } finally {
        await tester.pumpWidget(Container());
        await tester.pumpAndSettle(const Duration(seconds: 1));

        if (chart.view.webViewController is WebWebViewController &&
            (chart.view.webViewController as WebWebViewController).mounted) {
          (chart.view.webViewController as WebWebViewController).dispose();
        }
      }
    });
  });
